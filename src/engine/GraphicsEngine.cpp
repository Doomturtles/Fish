...
void GraphicsEngine::InitializeMedia() {
	Media::InitializeImages(renderer);
}
...
void GraphicsEngine::DrawAlert(Point2& l1, Point2& l2, const float& radius)
{
	...
	Vector2f* centralLine = new Vector2f(l1.x - l2.x, l1.y - l2.y);
	Vector2f* perpendicularLine = new Vector2f(centralLine->y, centralLine->x * -1);
	Vector2f* lineOffset = Vector2f::Normalise(*perpendicularLine);
	*lineOffset = *lineOffset * radius;
	Point2* lineOffsetP = new Point2(lineOffset->x, lineOffset->y);
	Point2* line1point1 = &(l1 + *lineOffsetP);
	Point2* line1point2 = &(l2 + *lineOffsetP);
	Point2* line2point1 = &(l1 - *lineOffsetP);
	Point2* line2point2 = &(l2 - *lineOffsetP);
	SDL_RenderDrawLine(renderer, line1point1->x, line1point1->y, line1point2->x, line1point2->y);
	SDL_RenderDrawLine(renderer, line2point1->x, line2point1->y, line2point2->x, line2point2->y);
	//curved lines on the end
	float angleOfAlert = toRadians(Vector2f::Vector2fToDegrees(*lineOffset));
	for (float i = angleOfAlert; i < angleOfAlert + M_PI; i += PI_OVER_180) {
		int x = (int)(l1.x + radius * cos(i));
		int y = (int)(l1.y + radius * sin(i));
		SDL_RenderDrawPoint(renderer, x, y);
	}
	for (float i = angleOfAlert; i > angleOfAlert - M_PI; i -= PI_OVER_180) {
