...
EventEngine::EventEngine() : running(true) {
	...
	std::copy(std::begin(keys), std::end(keys), std::begin(keysLastFrame));
	std::copy(std::begin(buttons), std::end(buttons), std::begin(buttonsLastFrame));
}
...
bool EventEngine::wasPressed(Key key) {
	return keysLastFrame[key];
}

bool EventEngine::wasPressed(Mouse btn) {
	return buttonsLastFrame[btn];
}
bool EventEngine::isDown(Key key) {
	return keys[key] && !keysLastFrame[key];
}
bool EventEngine::isDown(Mouse btn) {
	return buttons[btn] && !buttonsLastFrame[btn];
}
